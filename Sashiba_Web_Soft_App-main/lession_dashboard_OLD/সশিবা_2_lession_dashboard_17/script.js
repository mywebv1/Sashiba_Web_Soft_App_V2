...
function handleSave() {
  ...
  // ১. পুরো কন্টেন্টকে একটি মাস্টার টেবিলের ভেতর রাখা (যাতে লেআউট নড়ে না যায়)
  const content = `
  ...
  // ২. ইনলাইন স্টাইল দিয়ে ফরম্যাটিং ফিক্স করা
  const styledContent = content
    .replace(
      /class="meta-row"/g,
      'style="display: table; width: 100%; margin: 15px 0;"',
    )
    .replace(
      /<table/g,
    ...
    .replace(
      /<th/g,
      '<th style="border: 1px solid #000; padding: 10px; background-color: #f2f2f2; text-align: center;"',
    )
    .replace(/<td/g, '<td style="border: 1px solid #000; padding: 10px;"');
