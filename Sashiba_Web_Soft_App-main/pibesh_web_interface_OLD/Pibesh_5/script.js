...
function loadCategoryPage(category, subCategory) {
  currentCategory = category;
  currentSubCategory = subCategory;

  // Set breadcrumbs
  document.getElementById("bc-level").innerText = category;
  document.getElementById("bc-subject").innerText = subCategory;
...
// Fast Chapter select dropdown trigger
function handleFastChapterSelect(val) {
  if (!val) return;
  currentChapter = val;
  renderDashboardContent();
}
...
function renderChapterSidebar() {
  ...
  const dropdown = document.getElementById("fast-chapter-select");
  dropdown.innerHTML = '<option value="">দ্রুত অধ্যায় পরিবর্তন...</option>';
  chapters.forEach((chap) => {
    dropdown.innerHTML += `<option value="${chap}">${chap}</option>`;
  });
