...
function loadTopics(checkbox, safeId, chapter) {
  ...
  if (checkbox.checked) {
    ...
    // যদি ডাটাবেসে এই অধ্যায়ের জন্য আলাদা আলোচ্য বিষয় না থাকে, তবে ডাইনামিক তৈরি করো
    if (topics.length === 0) {
      var cleanName = chapter.replace(/অধ্যায়-\d+\s*:\s*/g, "").replace(/Unit-\d+\s*:\s*/g, "").trim();
      if (currentLang === "bn") {
        topics = [
          cleanName + " - বিস্তারিত আলোচনা",
          cleanName + " - মূল ভাব ও গুরুত্ব",
          cleanName + " - প্রয়োগ ও উদাহরণ",
          cleanName + " - সৃজনশীল চিন্তন ও সমাধান",
          cleanName + " - বহু নির্বাচনী প্রশ্নোত্তর"
        ];
      } else {
        topics = [
          cleanName + " - Detailed Discussion",
          cleanName + " - Core Concept & Importance",
          cleanName + " - Applications & Examples",
          cleanName + " - Creative Thinking & Problems",
          cleanName + " - Evaluation & Summary"
        ];
      }
    }

    var titleSuffix = currentLang === "bn" ? " এর আলোচ্য বিষয়সমূহ:" : " Topics:";
    var html = '<div id="tp-' + safeId + '" style="padding:10px;background:#fff;border-radius:8px;margin-top:6px;border:1px dashed #cbd5e1;">' +
               '<strong style="font-size:13px;">' + chapter + titleSuffix + '</strong><br>';
...
function generate() {
  ...
  // অধ্যায় সিলেক্ট করা হয়েছে কিনা পরীক্ষা করো
  var chapterChecks = document.querySelectorAll("#chapterContainer input[type='checkbox']:checked");
  if (chapterChecks.length === 0) {
    alert(currentLang === "bn"
      ? "দয়া করে অন্তত একটি অধ্যায় নির্বাচন করুন!"
      : "Please select at least one chapter!");
    return;
  }

  // আলোচ্য বিষয় সিলেক্ট করা হয়েছে কিনা পরীক্ষা করো
  var topicChecks = document.querySelectorAll("#topicContainer input[type='checkbox']:checked");
  if (topicChecks.length === 0) {
    alert(currentLang === "bn"
      ? "দয়া করে অধ্যায়ের অন্তর্গত অন্তত একটি আলোচ্য বিষয় নির্বাচন করুন!"
      : "Please select at least one topic under the selected chapter(s)!");
    return;
  }

  // সিলেক্টেড আলোচ্য বিষয়সমূহ সংগ্রহ করো
  var selectedTopics = [];
  topicChecks.forEach(function (tc) {
    selectedTopics.push(tc.parentElement.innerText.trim());
  });
  ...
  checks.forEach(function (c, idx) {
    ...
    for (var i = 1; i <= count; i++) {
      // আলোচ্য বিষয় রাউন্ড-রবিন অনুযায়ী নির্বাচন করো
      var currentTopic = selectedTopics[topicIndex % selectedTopics.length];
      topicIndex++;

      // নমুনা প্রশ্ন
      var qLabel = currentLang === "bn"
        ? "এখানে [" + currentTopic + "] বিষয়ের উপর " + type + "-\u098f\u09b0 " + i + " \u09a8\u0982 \u09a8\u09ae\u09c1\u09a8\u09be \u09aa\u09cd\u09b0\u09b6\u09cd\u09a8\u099f\u09bf \u09aa\u09cd\u09b0\u09a6\u09b0\u09cd\u09b6\u09bf\u09a4 \u09b9\u09ac\u09c7\u0964"
        : "Sample question " + i + " for " + type + " based on [" + currentTopic + "] will appear here.";
      qHtml += "<li style='margin-bottom:8px;'>" + qLabel + "</li>";

      // উত্তরপত্রের অংশ — ansSheet চেক থাকলেই তৈরি হবে
      if (showAns) {
        var ansLabel = currentLang === "bn"
          ? "নমুনা উত্তর: [" + currentTopic + "] বিষয়ের উপর " + type + "-\u098f\u09b0 " + i + " \u09a8\u0982 \u09aa\u09cd\u09b0\u09b6\u09cd\u09a8\u09c7\u09b0 \u09b8\u09a0\u09bf\u0995 \u0989\u09a4\u09cd\u09a4\u09b0 \u098f\u0996\u09be\u09a8\u09c7 \u09a5\u09be\u0995\u09ac\u09c7\u0964"
          : "Sample Answer: The correct answer for " + type + " Q" + i + " based on [" + currentTopic + "] will be here.";

        // AI ব্যাখ্যা — ansExpl চেক থাকলেই যোগ হবে
        var aiPart = "";
        if (showExpl) {
          var aiLabel = currentLang === "bn"
            ? "AI \u09ac\u09cd\u09af\u09be\u0996\u09cd\u09af\u09be: \u098f\u0987 \u0989\u09a4\u09cd\u09a4\u09b0\u099f\u09bf \u09b8\u09a0\u09bf\u0995 \u0995\u09be\u09b0\u09a3 \u098f\u099f\u09bf [" + currentTopic + "] বিষয়ের মূল ধারণার সাথে সম্পূর্ণ সামঞ্জস্যপূর্ণ।"
            : "AI Explanation: This answer is correct as it perfectly aligns with the core concept of [" + currentTopic + "].";
